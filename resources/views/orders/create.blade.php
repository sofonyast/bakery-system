...
@endif
<div class="max-w-lg">
    <h2 class="text-2xl font-bold text-amber-900 mb-6">New Order</h2>
    <form action="{{ route('orders.store') }}" method="POST" class="bg-white rounded-xl shadow p-6 space-y-4">
        @csrf
        
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Customer Name</label>
            <input name="customer_name" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" required>
        </div>
        <div>
